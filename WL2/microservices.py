...
fList = [s.strip() for s in open('/home/ram/api-as-service/grand-slam/isca-results/services/cpu/NLU/latency.txt').readlines()]
...
'''
## Simulator Data
ii = defaultdict(lambda  : defaultdict(int))
brange = 128
ilist = [1]
fList = [s.strip() for s in open('/home/ram/api-as-service/grand-slam/isca-results/services/cpu/SEQ/latency.txt').readlines()]
appname = 'seq'
...
for item in fList:
     ...
     if 'batch size' in item:
         ...
         service_data[appname][bsize][isize].append(float(item.split(' - ')[1]))
#print service_data
...
II.input_size = 1       #Sequence Learning
#print II.microservice_dict
